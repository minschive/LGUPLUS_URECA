...
-- where
select * from book where price = 7000; -- 모든 row 중 where 조건에 맞는 row만 추출
select * from book where price > 20000; -- 모든 row 중 where 조건에 맞는 row만 추출
select * from book where price <> 20000; -- 모든 row 중 where 조건에 맞는 row만 추출 (<>, != : 다른 조건)

select * from Customer where address = "영국 맨체스터";
select * from Orders where bookid > 5;
...
select *
  from book
  where publisher in ('굿스포츠', '대한미디어'); -- publisher가 in 다음의 집합에 포함되는 것 추출 (권장)
  
select *
  from book
  where publisher not in ('굿스포츠', '대한미디어'); -- publisher가 in 다음의 집합에 포함되지 않는 것 추출 (권장 X)
...
-- like
select * from book where bookname like '축구의 역사'; -- wildcard가 없으므로 = 과 동일한 비교
...
select * from book where price <= 10000 or price >= 30000; -- 복합조건이 3개 이상이면서 and 와 or가 섞여 있는 경우 가독성을 위해서 () 를 활용
