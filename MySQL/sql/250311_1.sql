...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin; -- 대소문자 구분 O

create table ci_table (
	user_id varchar(5) not null
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci; -- 대소문자 구분 X
...
-- 대소문자를 구분하지 않도록 만든 테이블 또는 스키마에서 대소문자를 구분하야 한다면
-- ci_table ('aaa')
select * from ci_table where user_id = 'ABC'; -- O
select * from ci_table where user_id = 'abc'; -- O
...
-- 조회조건 컬럼이 index를 가진 경우 오른쪽(value 부분)에 함수를 사용하는 게 바람직
