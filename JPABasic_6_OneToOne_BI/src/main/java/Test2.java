...
//        System.out.println(person);
        ...
        // fetch = FetchType.LAZY 로 변경해도 Person 필드를 채우기 위해서 Person 객체를 가져오는 join 문이 함께 실행된다. (Ownership 을 작지 않는 엔티티는 의미가 없다)

        // #3. @OneToOne(cascade = CascadeType.PERSIST, fetch = FetchType.LAZY) 설정 후
//        Person person = em.find(Person.class, 1);
