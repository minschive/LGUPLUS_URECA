...
//        Post p = em.find(Post.class, 1);
        ...
        // OneToMany 의 One 에 해당하는 Post 에 달린 Many 에 해당하는 Comment 가 매우 많을 수 있는 복수 개이므로
        // 기본 fetch 옵션은 LAZY
...
//        Comment c1 = em.find(Comment.class, 1);
        ...
        // #3. FetchType 설정 없이 Post 객체의 find, Post 객체의 comments 사용
//        Post p = em.find(Post.class, 1);
////        p.getComments(); // 참조변수만 가져오는 것으로 Comment 객체를 사용하는 코드 X
//
//        try {
//            Thread.sleep(5000);
//        } catch(Exception e) {
...
//        p.setComments(List.of(c3)); // 기존 연결 모두 삭제하고 c3 연결을 연결한다는 의미
        p.getComments().add(c3); // 기존 연결을 유지한 채, 새로운 c3 객체 추가
